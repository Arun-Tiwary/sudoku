...
const SudokuTest = ({ question, setHomeComponent, difficulty }) => {
  let inputRef = useRef([], []);
  ...
  // const roREf = arr.map((el) => inputRef.current[el]);
  // console.log(roREf, "roREf");
  ...
  const [state, setState] = useState(sudokuQuestion);
  console.log(state);
  ...
  return (
    <div className="sudoku-area">
              ...
              {arr.map((column, columnIndex) => (
                ...
                >
                  ...
                  <input
                    ...
                    value={
                      state[row][column] === 0 ? "" : state[row][column]
      ...
      <div className="answer">
        {selectArr.map((num) => (
          <p onClick={(e) => {}} className="answer-key" key={num}>
            {num}
          </p>
        ))}
      </div>
