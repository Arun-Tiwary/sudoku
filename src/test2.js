...
const Test2 = ({ question, setHomeComponent, difficulty }) => {
  //styling
  ...
  let globalGrid = deepCopy(question);
  const [gridValue, setGridvlue] = useState(globalGrid);
  ...
  function handleFocus(row, column) {
    let degraded = globalGrid[row][column];
    console.log("valueeee", degraded);
    setGridvlue(degraded);
    setSudokuQuestion(globalGrid);
  }

  function handleAnswer(num) {
    globalGrid[globalRow][globalColumn] = num;
    setSudokuQuestion(globalGrid);
    console.log(num);
  }
